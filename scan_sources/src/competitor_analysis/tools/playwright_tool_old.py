...
class ScreenshotTool(PlaywrightTool):
    ...
    def _run(self, **kwargs: Any) -> Any:
        ...
        # Use asyncio to run both operations in sequence
        async def run_sequence():
            async with async_playwright() as p:
                browser_type_name = navigate_kwargs.get("browser_type", "chromium")
                ...
                browser = await browser_instance.launch(headless=navigate_kwargs.get("headless", True))
                page = await browser.new_page()
                
                try:
                    # Navigate
                    url = navigate_kwargs.get("url")
                    wait_for = navigate_kwargs.get("wait_for")
                    await page.goto(url, wait_until="networkidle")
                    if wait_for:
                        await page.wait_for_selector(wait_for)
                    
                    # Take screenshot
                    path = screenshot_kwargs.get("path")
                    if not path:
                        import datetime
                        path = f"screenshot_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.png"
