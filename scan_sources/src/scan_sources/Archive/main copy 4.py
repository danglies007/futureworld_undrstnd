...
class ScanFlow(Flow):
    ...
    def research_market_forces(self):
        ...
        for i, market_force in enumerate(research_plan.market_forces, 1):
            ...
            # writer_inputs['market_force'] = market_force.model_dump()
            # writer_inputs.update(market_force.model_dump())
            # Explode the market force attributes into the top level of writer_inputs
            # for key, value in market_force.model_dump().items():
            #     writer_inputs[key] = value

            try:
                # Print detailed writer_inputs structure for better debugging
                print(f"Debug - writer_inputs type: {type(writer_inputs)}")
                print(f"Debug - writer_inputs keys: {list(writer_inputs.keys())}")
                
                # Add detailed tracing to help identify the exact issue
                import traceback
                
                print(f"Running research crew for market force {market_force_id}")
                result = ResearchCrew().crew().kickoff(inputs=writer_inputs, market_force=market_force).raw
