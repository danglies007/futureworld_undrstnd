...
class ScanFlow(Flow):
    ...
    @router(develop_research_plan)  # Remove wait_for_approval here
    def handle_research_plan_flow(self, result):
        ...
        if isinstance(result, Market_Force_Plan):
            ...
            return "create_new_research_plan"
        elif result == "research_plan_approved":
            # Plan has been approved, proceed to research
            return "research_plan_approved"
        return None
    ...
    def create_new_research_plan(self):
        ...
        # This is required as a safety to ensure we have a plan, could actually be deleted
    ...
    def wait_for_approval(self):
        ...
        if sys.stdin.isatty():
            ...
            if approval == 'y':
                ...
                # Directly resume the flow instead of returning a value
            ...
            # We're not in an interactive environment, so we'll wait for the web interface
    ...
    def research_market_forces(self):
        ...
        for i, market_force in enumerate(plan.market_forces, 1):
            ...
            # Add the market force attributes directly to the inputs
            writer_inputs.update(market_force.model_dump())
            
            try:
                # In research_market_forces method, before calling research_crew.crew().kickoff()
                print(f"Debug - Type of market_force being passed: {type(market_force)}")
                print(f"Debug - Market force model attributes: {market_force.model_dump() if hasattr(market_force, 'model_dump') else str(market_force)[:100]}")
                ...
                # Continue with next market force instead of returning early
                print(f"Moving to next market force...")
                continue

        # Return all processed market forces, even if some failed
