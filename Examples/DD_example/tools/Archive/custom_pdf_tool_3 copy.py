...
class CustomPDFSearchTool3(RagTool):
    ...
    def __init__(self, pdf: Optional[Union[str, List[str]]] = None, auto_load: bool = True, **kwargs):
        ...
        if pdfs:
            ...
            logger.info("No PDFs currently in reference directory")
    ...
    def _load_pdf(self, pdf_path: str) -> None:
        ...
        try:
            if not os.path.isabs(pdf_path):
                ...
                pdf_path = os.path.join(REFERENCE_DIR, pdf_path)
                logger.debug(f"Absolute path: {pdf_path}")
            
            if os.path.exists(pdf_path) and pdf_path not in self._loaded_pdfs:
